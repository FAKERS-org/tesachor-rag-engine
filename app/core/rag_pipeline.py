...
#         system_prompt = """You are an expert Phnom Penh tourism guide with deep knowledge of:
...
# Current date: March 17, 2026 (consider seasonal factors)"""
...
#         total_tokens = len(system_prompt.split()) + len(user_prompt.split()) + len(response.content.split())
...
#         # Normalize to 0-1 (assuming scores are cosine similarity 0.5-1.0)
#         normalized = max(0, (avg_score - 0.5) * 2)
...
# ----
# In app/core/rag_pipeline.py or app/main.py
...
rag_pipeline = RAGPipeline(
    ...
    llm_client=llm,           # HuggingFace
    embedding_client=embeddings,  # HuggingFace
    cache_client=cache,
    reranker=None
)